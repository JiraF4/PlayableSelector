...
class PS_MissionDescriptionObjectivesListUI : ScriptedWidgetComponent
{
	...
	void FillList()
	{
		...
		PS_ObjectiveManager objectiveManager = PS_ObjectiveManager.GetInstance();
		array<PS_Objective> objectives = {};
		objectiveManager.GetObjectives(objectives);
		...
		foreach (PS_Objective objective: objectives)
		{
			...
			handler.SetDescription(objective);
		...
		{
			if (!description.GetVisibleForFaction(factionKey) && factionKey != "")
				continue;
			if (!description.GetVisibleForEmptyFaction() && factionKey == "")
				continue;
