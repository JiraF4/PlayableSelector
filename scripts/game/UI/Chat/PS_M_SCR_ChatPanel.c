modded class SCR_ChatPanel : ScriptedWidgetComponent
{
	override protected void UpdateChatMessages()
	{
		...
		if (!gameMode.IsChatDisabled() || gameMode.GetState() == SCR_EGameModeState.GAME || factionKey == "" || playerRole == EPlayerRole.ADMINISTRATOR || playerRole == EPlayerRole.SESSION_ADMINISTRATOR || Replication.IsServer())
